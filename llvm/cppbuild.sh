...
case $PLATFORM in
    ...
    linux-ppc64le)
        ...
        ;;
    macosx-*)
        $CMAKE --build $LLVM_BUILD -DLLVM_CCACHE_BUILD=ON -DCMAKE_EXE_LINKER_FLAGS='-Wl,-rpath,@loader_path/' -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH -DLLVM_BUILD_LLVM_DYLIB=ON -DLLVM_LINK_LLVM_DYLIB=ON -DCMAKE_BUILD_TYPE=Release -DLLVM_TARGETS_TO_BUILD=all -DLLVM_ENABLE_LIBXML2=OFF -DLLVM_INCLUDE_TESTS=OFF -DLLVM_ENABLE_PROJECTS="$PROJECTS" ../llvm
        make -j $MAKEJ
        make install > /dev/null
        cp lib/LLVMPolly.so ../../lib/libLLVMPolly.so
        ;;
